...
class Check{
    friend istream& operator>>(istream& ins, Check& check){
        ...
        Money cAmount;
        bool cash;
        ins >> cNumber >> cAmount >> cash;
        check.checkNumber = cNumber;
        check.checkAmount = cAmount;
        check.cashed = cash;
        return ins;
    }
    
    friend ostream& operator<<(ostream& os, const Check& check){
        os << "Check Number: " << check.checkNumber << ", Amount: " << check.checkAmount << ", ";
        if(check.cashed){
          os << "Cashed";
        }else{
          os << "Not Cashed";
...
public:
    Check(): checkNumber(0), checkAmount(), cashed(false){}
    Check(int number, Money amount, bool cash){
        this->checkNumber = number;
        this->checkAmount = amount;
        this->cashed = cash;
    }
    void setCheckNumber(int cNumber){
        this->checkNumber = cNumber;
    }
    void setCheckAmount(Money cAmount){
        this->checkAmount = cAmount;
    }
    void setCashed(bool cash){
        this->cashed = cash;
    }
    int getCheckNumber(){
